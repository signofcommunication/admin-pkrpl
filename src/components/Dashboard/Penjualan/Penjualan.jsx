...
function Penjualan() {
  ...
  const { getAllDataPenjualan, deleteDataPenjualan } = useProvider();

  async function getSuppliers() {
    try {
      const {
        data: { data },
      } = await getAllDataPenjualan();
      setData(data);
    } catch (e) {
      console.log(e);
    }
              ...
              <Paper sx={{ width: "100%", overflow: "hidden" }}>
                <TableContainer component={Paper}>
                  <Table sx={{ minWidth: 650 }} aria-label="simple table">
                    <TableHead>
                      <TableRow>
                        <TableCell align="start">Id Penjualan</TableCell>
